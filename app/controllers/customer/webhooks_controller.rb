class Customer::WebhooksController < ApplicationController
  ...
  def create
    ...
    # 例外処理
    # begin の処理でエラーが発生した場合に、rescue を実行
    # json を parse できなかったとき、Stripe の署名が無効のときに 400 エラーを返す
    begin
    ...
    case event.type
    # heckout.session.completed イベントを受け取った際
    # checkout.session.completed イベントには、Session オブジェクトと呼ばれるものが含まれ、顧客とその支払いに関する詳細を取得することができる
  ...
  def create_order(session)
    Order.create!({
                    ...
                    # checkout.session.completed イベントを受信した時点で、決済は完了しているため
                    status: 'confirm_payment'
                  })
  end

  def create_order_details(order, line_item)
    # Stripe に登録された商品 id を取得
